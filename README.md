...
**v1.1.10 "Regalia"** | **FREE AND OPEN SOURCE** | **GPL v2**
...
- **v1.1.10:** the first god and the road to him. Mael'Keth is tuned so a level-30s party can win (the live fight log showed no party below level 48 ever had), the five floors before every Old God grow tougher, a god's buffs last a few rounds, its intro shows its real HP, and dialogue answers do what they promise. Companions are protected from special attacks like normal hits and tanks shield before they taunt. Items waiting for pack room reach alts, cannot be lost, arrive on /boss, and bequests from new teams reach their leader. The Inn lists every patron, PvP control can no longer chain, and Team HQ upgrades cost what the menu shows. See `DOCS/release-notes/RELEASE_NOTES_1.1.10.md`.
- **v1.1.9:** two player reports. The feet slot offers boots, not weapons, and armor "of Shielding" is no longer labelled a shield. Gear sets count the gear you already have: pieces from before v1.1.0 never recorded which template they dropped as, so the same Leather Cap could count or not depending on when it dropped. That template is now read from the name, never written into the save, and only when the name is one the game really generates for that piece, in any language and with any enchantment, compared against every item so the closest match wins. On the live server that switches on or raises 118 player set bonuses. See `DOCS/release-notes/RELEASE_NOTES_1.1.9.md`.
- **v1.1.8:** the website stops stalling. On a server with 400 players the statistics page read every player's save blob without an index, which stalled the whole web service for up to 83 seconds out of every 115, whether or not anyone was visiting, and froze the news feed, the API and the browser terminal along with it. Seven indexes carry the fields those pages read, and the worst stall is now 7 seconds with the service burning 4 percent of a core instead of 68. A server upgrading from an older release has its indexes rebuilt once so every install matches. JSON responses are compressed, which takes the dashboard's main payload from 2.2 MB to 400 KB, and the web service's memory ceiling is raised to what it actually needs. Releases also deploy the website itself now, which they never did before, so the site can no longer drift behind the game.
- **v1.1.7:** the economy holds. An item's stats and value are bounded wherever an item enters play, and items already over those bounds are repaired the next time they are loaded. Enchanting adds what it cost instead of half again the item's worth, and an item now remembers how often it was enchanted even while it sits in the backpack, so the five-enchant limit holds. Reforging is a bounded roll: three a day, a real price at every level, a value that follows its formula instead of ratcheting upward, and a ceiling tied to the best weapon the dungeon can drop for someone of your level. One PvP fight moves at most a capped amount of gold to its winner, on both sides of the fight, and the winner receives exactly what the loser lost. Salvage is priced from the gear the loser is actually wearing. The gold audit looks for what it used to miss.
...
**Status:** v1.1.10 "Regalia". The world is running. [Watch it live.](https://usurper-reborn.net)
