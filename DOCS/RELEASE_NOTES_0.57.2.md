...
Same treatment for the `[E]` equip action. When a companion is cycled to, the prompt reads `"(E)quip on {companion}"` instead of the generic `"(E)quip immediately"`, and the successful-equip confirmation line now appends ` — on {companion}` so the player can tell at a glance which party member ended up with the gear. Two new localization keys (`combat.loot_equip_on_companion_option`, `combat.loot_equipped_on_companion_suffix`) translated across all 5 languages.

Four total new keys (`combat.loot_take_to_companion_option`, `combat.loot_added_to_companion`, plus the two equip-side keys above) translated across all 5 languages. The original labels remain in place for when the real player is selected, so single-character pickups read exactly as before.
...
## Audit Pass Follow-ups

Comprehensive audit across all v0.57.2 changes surfaced four additional issues:

- **Noctura betrayal boss didn't set `BossContext`.** `HandleNocturaBetrayal` created a fresh `CombatEngine` and called `PlayerVsMonsters` directly without the context object, so the whole boss-protection layer (teammate 75% cap, spell immunity handling, divine armor) no-op'd during that fight — even though Noctura is an Old God. Now sets a minimal `BossContext { GodType = Noctura, AttacksPerRound = 1, CanSave = false }` before combat and clears it after. Phase immunity / AoE / channel mechanics don't engage unless their specific fields are configured, so the other Old-God-boss-specific systems stay inert for the betrayal fight (appropriate — the betrayal fight is a single-phase affair).

- **Inn ScoreEquipment gap: Cleric flails weren't preferred.** The v0.57.2 class-aware score boosted `WeaponType.Mace` ×1.4 for Clerics but omitted `WeaponType.Flail`, which are also canonically clerical. Added `|| item.WeaponType == WeaponType.Flail` to the Cleric mace bonus clause.

- **Inn ScoreEquipment gap: prestige classes had no preference.** Tidesworn / Wavecaller / Abysswarden fell through to the default score with no class bias. Tidesworn now shares the Warrior+Paladin tank preference (shield in off-hand ×3.0, 1H weapon ×0.3). Wavecaller shares the Magician+Sage+MysticShaman caster preference (staff ×1.4 in main-hand). Abysswarden shares the Assassin dagger preference (dagger ×1.5, non-dagger 1H ×0.7 — the class's signature abilities Umbral Step, Shadow Harvest, Backstab all favor daggers). Cyclebreaker and Voidreaver intentionally left fall-through because their mechanics don't key off specific weapon types.

- **Umbral Step guard inconsistency between paths.** Multi-monster ability path guarded the Hidden-consumption check with `actualDamage > 0`; single-monster path was missing that guard. Both paths are safe in practice because `actualDamage` is `Math.Max(1, ...)` upstream, but the inconsistency is worth fixing for code clarity and to avoid consuming Hidden on a zero-damage roll in some future refactor. Single-monster path now matches.
...
- `Scripts/Systems/CombatEngine.cs` — Combat loot prompt now shows `(T)ransfer to {name}'s inventory` when a companion is selected via `<`/`>`, and the post-transfer confirmation names the companion explicitly instead of the ambiguous `"added to inventory"`. Same treatment for `[E]`: prompt reads `(E)quip on {name}` and the equip-success line appends ` — on {name}` so the player can tell which party member now has the gear.
...
- `Scripts/Systems/OldGodBossSystem.cs` — `HandleNocturaBetrayal` now sets a minimal `BossContext { GodType = Noctura, AttacksPerRound = 1, CanSave = false }` before combat and clears it after, so the Old God boss-protection layer (teammate 75% cap, spell immunity handling, divine armor path) engages during the betrayal fight. Caught by the v0.57.2 audit — previously the betrayal combat was using the plain PlayerVsMonsters path with no boss context.
- `Scripts/Locations/InnLocation.cs` — `ApplyClassWeaponPreference` Cleric branch now also matches `WeaponType.Flail` for the ×1.4 main-hand preference. Prestige classes wired into existing preference buckets: Tidesworn → tank (shield ×3.0, 1H ×0.3 off-hand), Wavecaller → caster (staff ×1.4 main-hand), Abysswarden → dagger (dagger ×1.5, non-dagger 1H ×0.7).
- `Scripts/Systems/CombatEngine.cs` — Single-monster ability Umbral Step guard now matches multi-monster path: `stealthAbilityCritSingle = !abilityAlreadyCrit && actualDamage > 0 && player.HasStatus(StatusEffect.Hidden);` (added the `actualDamage > 0` clause).
...
- `Localization/en.json`, `es.json`, `fr.json`, `hu.json`, `it.json` — 19 new keys: `home.view_party_inv`, `team.menu_view_inventories`, `ui.cursed`, 12 `party_inv.*` keys covering the viewer UI, 2 `combat.loot_*_to_companion*` keys for the transfer-to-companion labels, and 2 `combat.loot_equip_on_companion_option` / `combat.loot_equipped_on_companion_suffix` keys for the equip-on-companion prompt and success-line suffix. Translated across all five supported languages.
