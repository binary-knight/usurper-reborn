...
A new **Music Shop** opens on Main Street, run by Melodia the Songweaver and her apprentice Cadence. Any class can hire lore-rich bard performances for combat buffs — 12 songs mixing tavern humor and dramatic ballads about the realm's history, its gods, and its people. Bards get a dedicated home for buying instruments, and Melodia herself is recruitable as the game's **5th companion** at level 20+. When she leaves to adventure with you, Cadence takes over the shop. The Music Shop also houses **Lore Songs of the Old Gods** — ancient ballads that unlock as you encounter each god, granting awakening points on first listen.

Two new weapon types join the arsenal: **Bows** for Rangers and **Instruments** for Bards. Classes now require specific weapons for their signature abilities — Rangers need a Bow for shot abilities, Assassins need a Dagger for blade techniques, and Magicians need a Staff to cast spells. The combat quickbar shows greyed-out abilities with clear reasons when the wrong weapon is equipped.

The **Weapon and Armor Shops** have been completely rebuilt with procedurally generated inventory covering levels 1-100, economy-scaled pricing, and class restriction tags. Two-handed weapons (35 templates) and shields (24 templates) now actually drop from dungeon monsters — they existed in the database but were never in the loot tables. Group loot uses a new round-robin system with pass-down chains.

**Compact Mode** lets players on phones, small terminals, and Steam Deck toggle space-efficient menus across all 15+ locations and combat screens. Three toggle methods (pre-login `[Z]`, preferences `[9]`, `/compact` command), touch-friendly number keys on Main Street, and full compatibility with Screen Reader Mode.
...
Multiple weapon and armor templates were restricted to a "Monk" class that doesn't exist in the game. Gis, Headbands, Sandals, and other items could never drop for any player. All "Monk"-exclusive items have been reassigned to appropriate classes: light armor (Gis) to Assassin/Barbarian, accessory pieces to Assassin/Ranger/Barbarian as thematically appropriate. Quarterstaff's Monk reference removed (still usable by Magician, Sage, Cleric).
...
- `Scripts/Core/EquipmentEnums.cs` — Added `Bow = 15` and `Instrument = 17` to `WeaponType` enum (two-handed ranged weapon; Bard musical instruments one-handed)
